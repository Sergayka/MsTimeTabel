...
data = []
for row in sheet.iter_rows(min_col=1, max_col=35, values_only=False):  # max_col теперь max_column
    row_data = []
    for cell in row:
...
for i in range(1, len(data) - 1, 2):
    ...
    for group_index in range(2, len(data[i])):
        # print(data[i])
        group_name = data[0][group_index][0]  # Название группы из первой строки (ячейка первого столбца текущей группы)
        # print(group_name)
        # break
        ...
        if merged:
            ...
            add_lesson("Четная неделя", group_name, current_day, time_slot, lesson)
            add_lesson("Нечетная неделя", group_name, current_day, time_slot, lesson)
        else:
            # Если ячейка не объединена, то одно занятие для четной недели, другое для нечетной
            add_lesson("Четная неделя", group_name, current_day, time_slot, lesson)
            add_lesson("Нечетная неделя", group_name, current_day, time_slot,
                       data[i + 1][group_index][0])  # Следующий урок для нечетной недели

#Создадим папку для сохранения файлов, если она не существует
output_dir = "schedule_jsons"
